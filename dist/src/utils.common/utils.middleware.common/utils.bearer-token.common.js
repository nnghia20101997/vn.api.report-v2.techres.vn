...
let AuthenticationMiddleware = class AuthenticationMiddleware {
    ...
    async use(req, res, next) {
        ...
        if (!bearerToken || bearerToken === '') {
            throw new common_1.HttpException(new utils_exception_common_1.ExceptionResponseDetail(common_1.HttpStatus.BAD_REQUEST, "Vui lòng truyền token"), common_1.HttpStatus.OK);
        }
        let decodeBearerTokenInterFace = await new utils_decode_token_common_1.DecodeToken().verifyBearerToken(bearerToken, "1235sd-5656sdf-@dfkdf-sdsjfdj");
        next();
    }
};
